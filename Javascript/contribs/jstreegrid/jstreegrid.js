...
(function ($) {
	...
	renderAWidth = function(node,tree) {
		...
		width = width - depth*18;//console.log(width)
		...
		_fn : { 
			_prepare_headers : function() {
				...
				for (i=0;i<cols.length;i++) {
					...
					borPadWidth = tr ? 1+6 : 6; // account for the borders and padding
                                       
					width -= borPadWidth;
                                        
                                        if(cols[i].group)
                                        {
                                            if(!group_html[cols[i].group])
                                            {
                                                //group_html[cols[i].group] = $("<div></div>").text('okoko');
                                                group_html[cols[i].group] = 
                                                {
                                                    ct : $("<div></div>").text('Teste'),
                                                    el : $("<div></div>")
                                                };
                                            }
                                ...
                                for(var tio in group_html)
                                {
                                    ...
                                    //group_html[tio].el.appendTo(group_html[tio].ct.css(conf));
                                }
